...
/*   Updated: 2024/06/25 09:50:21 by andymalgonn      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void handle_file_error(char *file, t_info *info, int pipefd[2], int *error) {
    if (!file) {
        mclose(info->fds[0]);
        mclose(pipefd[1]);
        *error = -1;
